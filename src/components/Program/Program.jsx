        ...
        const idInput = async () => {
            try {
                ...
                const { value: cod } = await Swal.fire({
                    title: `Ingrese el nuevo Programa de la Facultad de ${rolFacultad.facultad}`,
                    inputLabel: "Recuerde que el nombre del Programa no puede repetirse",
                    input: "text",
                    inputValue,
                    ...
                    inputValidator: (cod) => {
                        if (!cod) {
                            return "Debe ingresar el nombre del programa para continuar!";
                        } else {
                            const repetido = (rows.filter((row) => row.id === cod));
                            if (repetido.length > 0) {
                                return `Ya existe un Programa con ese Nombre ${cod} !`;
                            }
                        }
                    }
                });
...
const Program = () => {
    ...
    useEffect(() => {
        const downData = async () => {
            ...
            if (docSnap.exists()) {
                const listado = [...docSnap.data().programas];
                const programasEncontrados = [];
                listado.forEach((programa) => {
                    programasEncontrados.push({ id: programa, nombre: programa });
                });
                setRows(programasEncontrados);
            } else {
    ...
    const handleDeleteClick = (id) => () => {
        ...
        Swal.fire({
            title: `Seguro desea borrar el Programa ${id}?`,
            ...
            if (result.isConfirmed) {
                ...
                const upData = async () => {
                    await deleteDoc(doc(db, "areasXprograma", id));                                        
                    const registro = doc(db, "usuarioXfacultad", rolFacultad.id);
                    await updateDoc(registro, {
                        programas: arrayRemove(id)  // Elimina automáticamente el nuevo programa al arreglo de programas en el documento
                    });
                }
                upData();
                ...
                Swal.fire({
                    ...
                    icon: "success",
                    title: "Eliminado!",
                    text: `El Programa ${id} fue eliminado con éxito!`,
    ...
    const processRowUpdate = (newRow) => {
        ...
        const upData = async () => {
            if (newRow.nombre) {
                const registro = doc(db, "usuarioXfacultad", rolFacultad.id);                
                await updateDoc(registro, {
                    programas: arrayUnion(newRow.nombre)  // Agrega automáticamente el nuevo programa al arreglo de programas en el documento
                });
                await setDoc(doc(db,  "areasXprograma", newRow.nombre), {facultad:rolFacultad.id, areas:[]});
                Swal.fire({
                    position: "center-center",
                    icon: "success",
                ...
                Swal.fire({
                    icon: "error",
                    title: "Oops...",
                    html: `Se presentó un problema, el campo: <br><b>Denominación del Programa</b><br>es obligatorio`,
                });
            }
        }
        upData();
    ...
    const columns = [
        ...
        {
            field: 'nombre',
            headerName: 'Denominación del Programa',
            width: 480,
            editable: true
        },
        {
            ...
            <div style={{ display: "flex", justifyContent: "center", maxWidth: "60%" }}>
                ...
                >
                    <Typography gutterBottom variant="h5" component="div" sx={{ color: "#1876D1" }}>
                        Programas adscritos a la Facultad {rolFacultad.facultad}
                    </Typography>
                    <br />
